...
<div class="!font-mono" x-data="{
    ...
    @if ($listeningParty->end_time === null)
        <div class="flex items-center justify-center min-h-screen bg-indigo-50" wire:poll.5s>
            <div class="w-full max-w-2xl p-8 mx-8 bg-white rounded-lg shadow-lg">
                <div class="flex items-center justify-center space-x-8">
                    <div class="relative flex items-center justify-center w-16 h-16">
                        <span
                            class="absolute inline-flex rounded-full opacity-75 size-10 bg-indigo-400 animate-ping"></span>
                        <span
                            class="relative inline-flex items-center justify-center text-2xl font-bold text-white rounded-full size-12 bg-indigo-500">
                            🥵
                        </span>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="font-mono text-lg font-semibold text-slate-900">Creating your listening party</p>
                        <p class="mt-1 text-sm text-slate-600">
                            The podcaster room <span class="font-bold"> {{ $listeningParty->name }}</span> is being put
                            together...
                        </p>
                    </div>
                </div>
    ...
    @elseif($isFinished)
        <div class="flex items-center justify-center min-h-screen bg-indigo-50">
            <div class="w-full max-w-2xl p-8 mx-8 text-center bg-white rounded-lg shadow-lg">
                <h2 class="mb-4 font-mono text-2xl font-bold text-slate-900">This listening party has finished 🥲</h2>
                <p class="mt-2 text-slate-600">The podcaster room <span
    ...
    @else
        ...
        <div x-show="!isLive" class="flex items-center justify-center min-h-screen bg-indigo-50" x-cloak>
            <div class="relative w-full max-w-2xl p-6 bg-white rounded-lg shadow-lg">
                ...
                <div class="mt-6 text-center">
                    <p class="font-mono font-semibold tracking-tight text-slate-600">Starting in:</p>
                    <p class="font-mono text-4xl font-semibold tracking-wider text-indigo-700 mt-4" x-text="countdownText">
                    </p>
                </div>
                ...
                <h2 x-show="isReady"
                    class="mt-8 font-mono text-lg tracking-tight text-center text-slate-900 font-bolder">
                    Ready to start the podcaster party! Stay tuned. 🫶</h2>
                ...
                <div class="flex items-center justify-end mt-8">
                    <button @click="copyToClipboard();"
                        ...
                        <span x-show="copyNotification" class="tracking-tight text-indigo-500" x-cloak>Copied!</span>
        ...
        <div x-show="isLive" x-cloak class="flex items-center justify-center min-h-screen bg-indigo-50">
                            ...
                            <div class="grid grid-cols-6 gap-2">
                                @foreach (['👍', '❤️', '😂', '😮', '😢', '😡'] as $emoji)
                                    <button @click="addEmoji('{{ $emoji }}', $event)"
                                            class="p-2 text-2xl transition-colors rounded-full hover:bg-indigo-100">
                                        {{ $emoji }}
                                    </button>
                                @endforeach
                            </div>
